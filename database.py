...
class Database(object):
    ...
    def add_user(self, tweet_id, payment_hash, payment_request, checking_id, amount, ispaid, date, unretweet):
        if self.is_user_saved(tweet_id):
            return
        self._add_user(tweet_id, payment_hash, payment_request, checking_id, amount, ispaid, date, unretweet)
        
    def _add_user(self, tweet_id, payment_hash, payment_request, checking_id, amount, ispaid, date, unretweet):
        try:
            self.cursor.execute("INSERT INTO users VALUES (?, ?, ?, ?, ?, ?, ?, ?);", [str(tweet_id), payment_hash, payment_request, checking_id, amount, ispaid, date, unretweet])
            self.connection.commit()
